...
def scrape_route():
    # Files
    mapFile = 'imap/map_cache.csv'
    houseFile = 'apartment/house_cache.csv'
    ...
    # Search
    print('Current:', map_data.shape[0], 'All: ', len(apt_list))
    idx = 0
    for apt,addr in zip(apt_list, addr_list):
        ...
        try:
            ...
            print('Updating...(', idx,'/',len(apt_list),')')
            print('    ', apt)
            map_data.loc[apt] = goog_map(apt, addr)
            # print(map_data.loc[apt].to_frame().T)
        except:
            print('Unable to read more map. Save current data. Please try later.') # current ip being banned, need to wait for 24hrs
            break
